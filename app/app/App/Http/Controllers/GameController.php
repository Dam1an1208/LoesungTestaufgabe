...
class GameController extends Controller
{
    ...
    protected function someoneHasWon( GameBoard $game ): bool {
        ...
        for($i = 0; $i < 3; $i++)
        {
            if(    
                ...
                $game->getRow($i)->getSpace( 0 ) === $game->getRow($i)->getSpace( 3 ) &&
    ...
    protected function whoHasWon( GameBoard $game ): ?GamePlayer {
        ...
        if(!$this->someoneHasWon($game))
            return null;
        if($this->someoneHasWon($game) &&  $game->getLastPlayer()=== GamePlayer::Human)
            return GamePlayer::Human;
        if($this->someoneHasWon($game) && $game->getLastPlayer() === GamePlayer::Robot)
    ...
    protected function isAllowedToPlay( GameBoard $game, GamePlayer $player) : bool {
        ...
        if(($game->getLastPlayer() === GamePlayer::Robot && $player === GamePlayer::Robot) || ($game->getLastPlayer() === GamePlayer::Human && $player === GamePlayer::Human))
            return false;
        if(($game->getLastPlayer() === GamePlayer::Robot && $player === GamePlayer::Human) || ($game->getLastPlayer() === GamePlayer::Human && $player === GamePlayer::Robot))
            return true;
        return true;
    }
